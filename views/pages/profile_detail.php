...
// Tradutor de Atributos (Cabelo/Olhos/Etnia)
...
// NOVO: Decodificar detalhes do serviço
...
// Avaliações (Com Avatar)
$reviews = [];
$avgRating = 0;
$totalReviews = 0;
try {
    $stmtReviews = $db->getConnection()->prepare("
        SELECT r.*, u.profile_image as reviewer_avatar, u.display_name as reviewer_real_name
...
if ($level >= 3) $activeBadge = ['label' => 'VIP', 'color' => 'bg-pink-600 text-white border-pink-700 shadow-pink-200', 'icon' => 'fas fa-crown'];
...
<body class="text-slate-600 antialiased">
            ...
            <div class="panel p-5">
                ...
                <div class="grid grid-cols-2 gap-y-4 gap-x-2">
                    <div class="flex flex-col">
                        ...
                        <span class="font-semibold text-slate-700 text-sm"><?= $age ?></span>
                    </div>
                    <div class="flex flex-col">
                    ...
                    <?php if(!empty($profile['cup_size'])): ?>
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Seios</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= htmlspecialchars($profile['cup_size']) ?></span>
                    </div>
                    <?php endif; ?>
                    
                    <?php if(isset($profile['silicone'])): ?>
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Silicone</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= $mapSimNao[$profile['silicone']] ?></span>
                    </div>
                    <?php endif; ?>

                    <?php if(!empty($profile['shaving'])): ?>
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Depilação</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= $mapShaving[$profile['shaving']] ?? '--' ?></span>
                    </div>
                    <?php endif; ?>
                ...
                <div class="border-t border-slate-100 mt-4 pt-4 grid grid-cols-2 gap-y-4 gap-x-2">
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Tatuagem</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= $mapSimNao[$profile['tattoos'] ?? 0] ?></span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Fuma</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= $mapSmoker[$profile['smoker'] ?? 'no'] ?></span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-400">Bebe</span>
                        <span class="font-semibold text-slate-700 text-sm"><?= $mapDrinker[$profile['drinker'] ?? 'no'] ?></span>
                    </div>
                </div>
            </div>
        ...
        <main class="lg:col-span-8 space-y-6">
            ...
            <section class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="panel p-5 border-l-4 <?= $profile['incall_available'] ? 'border-pink-500' : 'border-slate-300 opacity-75' ?>">
                    ...
                    <?php if($profile['incall_available']): ?>
                        <ul class="space-y-2 text-sm text-slate-600 mt-2 pt-2 border-t border-slate-50">
            ...
            <section class="panel p-6">
                ...
                <?php elseif($profile['show_as_night']): ?>
                    <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center gap-4 text-white">
                        ...
                        <div>
                            <h4 class="font-bold text-white text-sm">Plantão Noturno</h4>
                            <p class="text-slate-400 text-xs">Atendimento preferencial à noite.</p>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="overflow-x-auto">
                        <div class="flex gap-2 min-w-max">
                            ...
                            foreach($daysMap as $key => $label): 
                                ...
                                $isActive = isset($day['active']) && $day['active'] == '1';
                            ?>
                            <div class="flex-1 min-w-[80px] p-3 rounded-lg border text-center <?= $isActive ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-100 opacity-60' ?>">
                                ...
                                <?php if($isActive): ?>
                                    <span class="block text-xs font-bold text-slate-800"><?= $day['start'] ?></span>
                                    <span class="block text-[10px] text-slate-400">até</span>
                                    <span class="block text-xs font-bold text-slate-800"><?= $day['end'] ?></span>
                                <?php else: ?>
                                    <span class="block text-xs font-medium text-slate-300">Folga</span>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; ?>
                        </div>
            ...
            <section class="panel p-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="font-display font-bold text-lg text-slate-800 flex items-center gap-2">
                        <i class="far fa-comments text-pink-500"></i> Avaliações Recentes
                    </h3>
                    <?php if(isset($_SESSION['user_id']) && $_SESSION['user_role'] === 'member'): ?>
                        <button onclick="document.getElementById('reviewForm').classList.toggle('hidden')" class="text-xs font-bold text-primary hover:underline">Avaliar</button>
                    <?php endif; ?>
                </div>

                <?php if(isset($_SESSION['user_id']) && $_SESSION['user_role'] === 'member'): ?>
                <div id="reviewForm" class="hidden mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <form onsubmit="submitReview(event)">
                        <input type="hidden" name="profile_id" value="<?= $profile['id'] ?>">
                        <input type="hidden" name="rating" id="rating-value" required>
                        <div class="flex gap-2 text-2xl text-slate-300 mb-3" id="star-input">
                            <i class="far fa-star cursor-pointer hover:text-yellow-400 transition" onclick="setRating(1)"></i>
                            <i class="far fa-star cursor-pointer hover:text-yellow-400 transition" onclick="setRating(2)"></i>
                            <i class="far fa-star cursor-pointer hover:text-yellow-400 transition" onclick="setRating(3)"></i>
                            <i class="far fa-star cursor-pointer hover:text-yellow-400 transition" onclick="setRating(4)"></i>
                            <i class="far fa-star cursor-pointer hover:text-yellow-400 transition" onclick="setRating(5)"></i>
                        </div>
                        <textarea name="comment" rows="2" class="w-full text-sm p-3 rounded-lg border border-slate-200 focus:outline-none focus:border-primary mb-2" placeholder="Conte sua experiência..." required></textarea>
                        <div class="text-right">
                            <button class="bg-slate-900 text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-slate-800 transition">Enviar Avaliação</button>
                        </div>
                    </form>
                </div>
                <?php endif; ?>
                ...
                <?php if(empty($reviews)): ?>
                    <p class="text-slate-400 text-sm text-center py-4">Nenhuma avaliação ainda.</p>
                <?php else: ?>
                    <div class="space-y-4">
                        <?php foreach($reviews as $rev): 
                            // Lógica para avatar do membro
                            $revImg = !empty($rev['reviewer_avatar']) ? url('/' . $rev['reviewer_avatar']) : null;
                            $revInitial = strtoupper(substr($rev['reviewer_real_name'] ?? $rev['reviewer_name'] ?? 'M', 0, 1));
                        ?>
                        <div class="border-b border-slate-50 pb-4 last:border-0 last:pb-0">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-3">
                                    <?php if($revImg): ?>
                                        <img src="<?= $revImg ?>" class="w-10 h-10 rounded-full object-cover border border-slate-200">
                                    <?php else: ?>
                                    ...
                                    <div>
                                        <span class="font-bold text-slate-700 text-sm block"><?= htmlspecialchars($rev['reviewer_real_name'] ?? $rev['reviewer_name'] ?? 'Membro') ?></span>
                                        <span class="text-[10px] text-slate-400 block"><?= date('d/m/Y', strtotime($rev['created_at'])) ?></span>
                                    </div>
                                </div>
                                
                                <div class="flex text-yellow-400 text-[10px]">
                                    <?php for($i=0; $i<5; $i++) echo ($i < $rev['rating']) ? '<i class="fas fa-star"></i>' : '<i class="far fa-star text-slate-300"></i>'; ?>
                                </div>
                            </div>
                            
                            <p class="text-slate-600 text-xs italic ml-13 pl-13 mt-1">"<?= htmlspecialchars($rev['comment']) ?>"</p>
                            
                            <?php if(!empty($rev['reply'])): ?>
                                <div class="mt-3 ml-12 pl-3 border-l-2 border-primary/30 bg-pink-50/30 p-2 rounded-r-lg flex gap-2">
                                    <img src="<?= $profileImg ?>" class="w-6 h-6 rounded-full object-cover border border-pink-200">
                                    <div>
                                        <p class="text-[9px] text-primary font-bold uppercase">Resposta:</p>
                                        <p class="text-xs text-slate-500"><?= htmlspecialchars($rev['reply']) ?></p>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
    ...
    <footer class="bg-white border-t border-slate-100 py-8 mt-12">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <p class="text-slate-400 text-xs">&copy; <?= date('Y') ?> TOPModel. Todos os direitos reservados.</p>
        </div>
    </footer>
    ...
    <script>
        ...
        function changeImage(dir) {
            if (currentIndex === -1 && galleryImages.length > 0) currentIndex = 0;
            else {
        ...
        function updateNavButtons() {
            ...
            document.getElementById('prevBtn').style.display = hasGallery ? 'flex' : 'none';
            document.getElementById('nextBtn').style.display = hasGallery ? 'flex' : 'none';
        }
