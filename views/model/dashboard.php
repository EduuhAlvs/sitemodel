...
if (isset($_GET['action']) && $_GET['action'] === 'load_history') {
    ...
    try {
        ...
        foreach($rows as $row) {
            ...
            $statusLabel = 'Pendente'; $statusClass = 'bg-slate-100 text-slate-500';
...
// RECARREGAR DADOS DO PERFIL (Garante que medidas salvas apareçam mesmo que o Controller não traga)
// Isso resolve o problema visual de "não salvou" se o controller estiver desatualizado
$stmtProfile = $conn->prepare("SELECT * FROM profiles WHERE id = ?");
$stmtProfile->execute([$profile['id']]);
$freshProfile = $stmtProfile->fetch(\PDO::FETCH_ASSOC);
if ($freshProfile) {
    $profile = array_merge($profile, $freshProfile);
}
...
$activePlans = [];
try {
    $stmtPlans = $conn->prepare("SELECT s.*, DATEDIFF(s.expires_at, NOW()) as days_left FROM subscriptions s WHERE s.profile_id = ? AND s.payment_status = 'paid' AND s.expires_at > NOW()");
    ...
    foreach ($rawPlans as $p) {
        $name = getPlanName($p['price_paid'], $p['plan_id']);
        if (isset($groupedPlans[$name])) {
            if (strtotime($p['expires_at']) > strtotime($groupedPlans[$name]['expires_at'])) $groupedPlans[$name]['expires_at'] = $p['expires_at'];
        } else {
            $groupedPlans[$name] = ['plan_name' => $name, 'expires_at' => $p['expires_at'], 'weight' => $hierarchyWeights[$name] ?? 0];
        }
    }
...
<head>
    ...
    <script>
        tailwind.config = { theme: { extend: { fontFamily: { sans: ['Plus Jakarta Sans', 'sans-serif'], display: ['Outfit', 'sans-serif'] }, colors: { primary: '#db2777', secondary: '#4f46e5', surface: '#ffffff' } } } }
...
<body class="text-slate-600 antialiased">
    <nav class="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 h-16">
        <div class="max-w-7xl mx-auto px-4 h-full flex justify-between items-center">
            <div class="flex items-center gap-2"><span class="font-display font-bold text-xl tracking-tight text-slate-900">TOP<span class="text-pink-600">Model</span></span><span class="bg-slate-100 text-slate-500 text-[10px] font-bold px-2 py-0.5 rounded border border-slate-200 uppercase tracking-wide">Painel Modelo</span></div>
            <div class="flex items-center gap-4"><a href="<?= url('/logout') ?>" class="text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition uppercase tracking-wide">Sair</a></div>
        </div>
    </nav>
    <div class="max-w-7xl mx-auto px-4 pt-24 pb-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <aside class="lg:col-span-3 space-y-5 sidebar-sticky">
            <div class="panel p-4 bg-white">
                <div class="flex justify-between items-center mb-3"><span class="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Meus Perfis (<?= count($myProfiles) ?>)</span><?php if($canCreateMore): ?><a href="<?= url('/perfil/criar') ?>" class="text-[10px] bg-green-50 text-green-600 px-2 py-0.5 rounded font-bold">+ Novo</a><?php else: ?><a href="<?= url('/planos?msg=slots_full') ?>" class="text-[10px] bg-pink-50 text-pink-600 px-2 py-0.5 rounded font-bold">+ Comprar Vaga</a><?php endif; ?></div>
                <div class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                    <?php foreach($myProfiles as $p): $isActive = ($p['id'] == $profile['id']); $pImg = !empty($p['profile_image']) ? url('/' . $p['profile_image']) : 'https://ui-avatars.com/api/?name='.urlencode($p['display_name']).'&background=cbd5e1&color=fff&size=64'; 
                        $isOnline = false; if ($isActive) { $isOnline = $isVisuallyOnline; } else { $stmtCheck = $conn->prepare("SELECT COUNT(*) FROM subscriptions WHERE profile_id = ? AND payment_status = 'paid' AND expires_at > NOW()"); $stmtCheck->execute([$p['id']]); $isOnline = ($stmtCheck->fetchColumn() > 0) || ((int)($p['current_plan_level'] ?? 0) > 0); } ?>
                    <a href="?profile_id=<?= $p['id'] ?>" class="profile-selector-item <?= $isActive ? 'active' : '' ?>">
                        <img src="<?= $pImg ?>" class="w-8 h-8 rounded-full object-cover border border-slate-200"><div class="flex-1 min-w-0"><p class="text-xs font-medium truncate name"><?= htmlspecialchars($p['display_name']) ?></p><?php if($isOnline): ?><p class="text-[10px] truncate text-green-600 font-bold">● Online</p><?php else: ?><p class="text-[10px] truncate text-slate-400">○ Offline</p><?php endif; ?></div><?php if($isActive): ?><i class="fas fa-check text-pink-500 text-xs"></i><?php endif; ?>
                    </a>
                    <?php endforeach; ?>
                </div>
            </div>
            <div class="panel p-6 text-center group bg-white border-t-4 border-pink-500">
                <div class="relative inline-block mb-3"><img src="<?= $profileImg ?>" class="w-20 h-20 rounded-full object-cover ring-2 ring-offset-2 ring-pink-500 group-hover:scale-105 transition duration-300"><?php if($isVisuallyOnline): ?><div class="absolute bottom-0 right-0 w-5 h-5 bg-green-500 border-4 border-white rounded-full"></div><?php else: ?><div class="absolute bottom-0 right-0 w-5 h-5 bg-slate-400 border-4 border-white rounded-full"></div><?php endif; ?></div>
                ...
                <?php if($isVisuallyOnline): ?><div class="mt-4 inline-block px-3 py-1 bg-green-50 text-green-700 text-[10px] font-bold rounded-full border border-green-100"><i class="fas fa-check-circle"></i> PERFIL ATIVO</div><?php else: ?><a href="<?= url('/planos?profile_id='.$profile['id']) ?>" class="block w-full py-2 mt-4 rounded-lg bg-slate-900 text-white font-bold text-xs hover:bg-slate-700 transition">Ativar Perfil (Offline)</a><?php endif; ?>
            </div>
            <div class="panel hidden lg:block overflow-hidden py-1"><nav class="flex flex-col text-sm font-medium text-slate-600"><a href="#plans" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-layer-group w-4 text-center"></i> Planos</a><a href="#photos" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-camera w-4 text-center"></i> Fotos</a><a href="#personal" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-user w-4 text-center"></i> Dados</a><a href="#appearance" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-magic w-4 text-center"></i> Aparência</a><a href="#reviews" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-star w-4 text-center"></i> Avaliações</a><a href="#service" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-map-marker-alt w-4 text-center"></i> Serviços</a><a href="#schedule" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-clock w-4 text-center"></i> Horários</a><a href="#history" class="px-5 py-2.5 hover:bg-slate-50 hover:text-pink-600 transition flex items-center gap-3"><i class="fas fa-history w-4 text-center"></i> Histórico</a></nav></div>
        </aside>
        <main class="lg:col-span-9 space-y-6">
            
            <section id="plans" class="scroll-mt-24">
                <div class="flex justify-between items-center mb-3 px-1"><h3 class="font-display font-bold text-lg text-slate-800">Assinaturas Ativas</h3><a href="<?= url('/planos?profile_id='.$profile['id']) ?>" class="text-[10px] font-bold bg-pink-50 text-pink-600 px-3 py-1 rounded hover:bg-pink-100 transition uppercase tracking-wide">Comprar +</a></div>
                <?php if (empty($activePlans)): ?><div class="panel p-6 text-center border-dashed border-slate-300"><p class="text-slate-500 text-sm font-medium">Este perfil está no modo Gratuito (Offline).</p><a href="<?= url('/planos?profile_id='.$profile['id']) ?>" class="text-pink-600 text-xs font-bold hover:underline mt-1 inline-block">Ativar VIP para ficar Online</a></div>
                <?php else: ?><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"><?php foreach($activePlans as $index => $plan): $isMain = ($index === 0); $statusLabel = $isMain ? 'VIGENTE' : 'RESERVA'; $statusColor = $isMain ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'; $cardBorder = $isMain ? 'ring-1 ring-pink-500 bg-pink-50/20' : 'bg-white'; ?><div class="panel p-4 flex flex-col justify-between h-full <?= $cardBorder ?>"><div><div class="flex justify-between items-start mb-2"><span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 text-slate-600 font-bold text-xs"><?= strtoupper(substr($plan['plan_name'], 6, 1)) ?></span><span class="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded <?= $statusColor ?>"><?= $statusLabel ?></span></div><h4 class="font-bold text-slate-800 text-sm"><?= htmlspecialchars($plan['plan_name']) ?></h4><p class="text-[10px] text-slate-400 mt-0.5">Válido até: <?= date('d/m/Y', strtotime($plan['expires_at'])) ?></p></div><div class="mt-3 pt-3 border-t border-slate-100/50"><div class="flex items-baseline gap-1"><span class="text-xl font-bold text-slate-800"><?= $plan['days_left'] ?></span><span class="text-[10px] text-slate-500 font-medium">dias restantes</span></div></div></div><?php endforeach; ?></div><?php endif; ?>
            </section>
            ...
            <section id="photos" class="panel p-6 scroll-mt-24 relative bg-white">
                ...
                <div id="drop-zone" class="border border-dashed border-slate-300 bg-slate-50 rounded-xl p-6 text-center cursor-pointer hover:bg-white hover:border-pink-400 hover:shadow-sm transition group" onclick="document.getElementById('photoInput').click()"><input type="file" id="photoInput" multiple accept="image/*" class="hidden" onchange="handleFiles(this.files)"><div class="flex flex-col items-center gap-2"><i class="fas fa-cloud-upload-alt text-2xl text-slate-300 group-hover:text-pink-500 transition"></i><div><p class="text-sm font-semibold text-slate-600">Clique ou arraste fotos</p><p class="text-[10px] text-slate-400">JPG, PNG, WebP (Max 5MB)</p></div></div><div id="loading-overlay" class="hidden mt-3"><div class="h-1 w-full bg-slate-200 rounded-full overflow-hidden"><div id="progress-bar" class="h-full bg-pink-500 w-0 transition-all duration-300"></div></div></div></div>
                <div id="photoGrid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-5"><?php if (!empty($photos)): foreach($photos as $photo): ?><div class="relative group aspect-[3/4] rounded-lg overflow-hidden bg-slate-100 shadow-sm border border-slate-200" id="photo-<?= $photo['id'] ?>"><img src="<?= url('/' . $photo['file_path']) ?>" class="w-full h-full object-cover transition duration-500 group-hover:scale-105"><span class="absolute top-1.5 left-1.5 px-1.5 py-0.5 text-[8px] font-bold text-white rounded backdrop-blur-md <?= $photo['is_approved'] ? 'bg-green-500/80' : 'bg-yellow-500/80' ?>"><?= $photo['is_approved'] ? 'OK' : 'ANÁLISE' ?></span><button onclick="deletePhoto(<?= $photo['id'] ?>)" class="absolute top-1.5 right-1.5 bg-white text-red-500 w-5 h-5 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition shadow hover:scale-110"><i class="fas fa-times text-[10px]"></i></button></div><?php endforeach; endif; ?></div>
            </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <section id="personal" class="panel p-6 scroll-mt-24 relative bg-white">
                    ...
                    <form onsubmit="saveCard(event, 'bio')" class="space-y-4">
                        ...
                        <div><label class="block text-xs font-semibold text-slate-500 mb-1">Nacionalidade</label><input type="text" name="nationality" value="<?= $profile['nationality'] ?>" class="input-std"></div>
            ...
            <section id="history" class="panel overflow-hidden scroll-mt-24">
                <div class="p-6 border-b border-slate-100 bg-white"><h3 class="font-display font-bold text-lg text-slate-800 flex items-center gap-2"><i class="fas fa-history text-pink-500"></i> Histórico Detalhado</h3></div>
                <div class="w-full">
                    <table class="w-full text-left text-sm text-slate-600">
                        <thead class="hidden md:table-header-group bg-slate-50 text-slate-500 uppercase text-[10px] font-bold tracking-wider border-b border-slate-200">
                            <tr><th class="px-6 py-3 border-b border-slate-200">Data</th><th class="px-6 py-3 border-b border-slate-200">Plano</th><th class="px-6 py-3 border-b border-slate-200">Valor</th><th class="px-6 py-3 border-b border-slate-200">Status</th><th class="px-6 py-3 border-b border-slate-200 text-right">Validade</th></tr>
                        </thead>
                        <tbody id="history-body" class="divide-y divide-slate-100 bg-white"></tbody>
                    </table>
                </div>
                <div id="history-loading" class="p-4 text-center text-xs text-slate-400 hidden">Carregando...</div>
                <div class="p-4 text-center bg-slate-50 border-t border-slate-100"><button id="btn-load-history" onclick="loadHistory()" class="text-xs font-bold text-pink-600 hover:text-pink-700 uppercase tracking-wide">Carregar Mais</button></div>
            </section>
        </main>
    </div>

    <div id="vip-success-modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4"><div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onclick="closeVipModal()"></div><div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center modal-animate border-2 border-green-500"><div class="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4 text-3xl shadow-sm"><i class="fas fa-check"></i></div><h2 class="text-2xl font-display font-bold text-slate-800 mb-2">Parabéns!</h2><p class="text-slate-500 text-sm mb-6">Você adquiriu o plano <strong id="modal-plan-name" class="text-slate-900">VIP</strong> com sucesso.<br>Seu perfil está destacado por <strong id="modal-days" class="text-green-600">30</strong> dias!</p><button onclick="closeVipModal()" class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl transition shadow-lg transform active:scale-95">Incrível, vamos lá!</button></div></div>
            ...
            if (json.success) {
                if (json.data.length === 0 && historyPage === 1) {
                    ...
                    json.data.forEach(item => {
                        ...
                        tr.innerHTML = `<td class="flex justify-between md:table-cell md:px-6 md:py-4 md:whitespace-nowrap"><span class="md:hidden text-xs font-bold text-slate-500 uppercase w-20">Data</span><div class="text-right md:text-left"><span class="font-bold text-slate-700 block text-sm md:text-base">${item.date}</span><span class="text-[10px] text-slate-400">${item.time}</span></div></td><td class="flex justify-between items-center md:table-cell md:px-6 md:py-4"><span class="md:hidden text-xs font-bold text-slate-500 uppercase w-20">Plano</span><span class="font-medium text-slate-800 text-sm md:text-xs text-right md:text-left">${item.plan_name}</span></td><td class="flex justify-between items-center md:table-cell md:px-6 md:py-4"><span class="md:hidden text-xs font-bold text-slate-500 uppercase w-20">Valor</span><span class="text-sm md:text-xs font-bold text-slate-600 text-right md:text-left">R$ ${item.price}</span></td><td class="flex justify-between items-center md:table-cell md:px-6 md:py-4"><span class="md:hidden text-xs font-bold text-slate-500 uppercase w-20">Status</span><div class="text-right md:text-left">${item.status_html}</div></td><td class="flex justify-between items-center md:table-cell md:px-6 md:py-4"><span class="md:hidden text-xs font-bold text-slate-500 uppercase w-20">Validade</span><div class="text-sm md:text-xs text-right md:text-left">${item.validity_html}</div></td>`;
                        tableBody.appendChild(tr);
                    });
                    historyPage++;
                    if (!json.has_more) btn.classList.add('hidden');
                }
            }
        } catch (e) {} finally { isLoadingHistory = false; btn.disabled = false; btn.classList.remove('opacity-50'); loader.classList.add('hidden'); }
    }

    function closeVipModal() { document.getElementById('vip-success-modal').classList.add('hidden'); }
    async function replyReview(e, id) { e.preventDefault(); const text = e.target.querySelector('input').value; if(!text) return; try { const res = await fetch(`${BASE_URL}/api/reviews/reply`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({review_id: id, reply: text, profile_id: CURRENT_PROFILE_ID}) }); if((await res.json()).success) location.reload(); } catch(err) { alert('Erro ao responder.'); } }
    async function deleteReview(id) { if(!confirm('Apagar avaliação?')) return; try { const res = await fetch(`${BASE_URL}/api/reviews/delete`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({review_id: id, profile_id: CURRENT_PROFILE_ID}) }); if((await res.json()).success) location.reload(); } catch(err) { alert('Erro ao apagar.'); } }
    ...
    async function saveCard(event, sectionName) {
        ...
        // CORREÇÃO CRÍTICA PARA MEDIDAS: Envia null se vazio para não quebrar SQL INT
    ...
    async function saveSchedule(e) {
        ...
        ['seg','ter','qua','qui','sex','sab','dom'].forEach(d => { const chk = form.querySelector(`input[name="hours[${d}][active]"]`); if(chk && chk.checked) { workingHoursObj[d] = { active: 1, start: fd.get(`hours[${d}][start]`), end: fd.get(`hours[${d}][end]`) }; } });
