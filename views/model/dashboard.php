...
<body class="text-slate-600 antialiased">
                        ...
                        <div class="panel p-4 flex flex-col justify-between h-full <?= $isMain ? 'ring-1 ring-pink-500 bg-pink-50/20' : 'bg-white' ?>">
                            <div>
                                <div class="flex justify-between items-start mb-2">
                                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 text-slate-600 font-bold text-xs"><?= substr($plan['plan_name'], 0, 1) ?></span>
                                    <span class="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded <?= $isMain ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500' ?>"><?= $isMain ? 'Vigente' : 'Reserva' ?></span>
                                </div>
                ...
                <section id="appearance" class="panel p-6 scroll-mt-24 relative bg-white">
                    ...
                    <form onsubmit="saveCard(event, 'appearance')" class="grid grid-cols-2 gap-3">
                        ...
                        <div class="col-span-2 flex items-center justify-between border-t border-slate-100 pt-3 mt-1">
                            ...
                            <div class="flex gap-4"><label class="flex items-center gap-1 cursor-pointer"><input type="radio" name="tattoos" value="1" <?= $profile['tattoos']?'checked':'' ?> class="chk-custom"> <span class="text-sm">Sim</span></label><label class="flex items-center gap-1 cursor-pointer"><input type="radio" name="tattoos" value="0" <?= !$profile['tattoos']?'checked':'' ?> class="chk-custom"> <span class="text-sm">Não</span></label></div>
                        </div>
    ...
    <div id="vip-success-modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center modal-animate border-2 border-green-500">
            ...
            <p class="text-slate-500 text-sm mb-6">
                Você adquiriu o plano <strong id="modal-plan-name" class="text-slate-900">VIP</strong> com sucesso.
                <br>Seu perfil está destacado por <strong id="modal-days" class="text-green-600">30</strong> dias!
            </p>
            
            <button onclick="closeVipModal()" class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl transition shadow-lg transform active:scale-95">
                Incrível, vamos lá!
            </button>
        </div>
    </div>
    ...
    <script>
    // Lógica do Modal de Sucesso VIP
    document.addEventListener('DOMContentLoaded', () => {
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('msg') === 'vip_success') {
            const planName = urlParams.get('plan_name') || 'VIP';
            const days = urlParams.get('days') || '30';
            
            document.getElementById('modal-plan-name').textContent = decodeURIComponent(planName);
            document.getElementById('modal-days').textContent = days;
            document.getElementById('vip-success-modal').classList.remove('hidden');
            
            // Limpa a URL para não mostrar o modal novamente ao atualizar
    ...
    async function saveCard(event, sectionName) {
        // ... (código existente da função saveCard) ...
    ...
    async function saveSchedule(e) {
        // ... (código existente da função saveSchedule) ...
        e.preventDefault();
        const form = e.target;
        const btn = form.querySelector('button');
        const oldText = btn.innerText; btn.innerText = "..."; btn.disabled = true;
        ...
        ['seg','ter','qua','qui','sex','sab','dom'].forEach(d => {
            ...
            if(chk && chk.checked) {
                workingHoursObj[d] = { active: 1, start: fd.get(`hours[${d}][start]`), end: fd.get(`hours[${d}][end]`) };
            }
        });
    ...
    // ... (restante das funções JS mantidas) ...
